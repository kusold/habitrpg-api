...
function HabitRpg(userId, apiKey, apiUrl) {
  ...
  this.user = {
    ...
    getUser: function(cb) {
      var req = self.buildRequest('GET', '/user');
      req.end(cb);
    },
    createTag: function(tag, cb) {
      var req = self.buildRequest('POST', '/user/tags', tag);
      req.end(cb);
    },
    updateTag: function(id, tag, cb) {
      var req = self.buildRequest('PUT', '/user/tags/' + id, tag);
      req.end(cb);
    },
    deleteTag: function(id, cb) {
      var req = self.buildRequest('DEL', '/user/tags/' + id);
      req.end(cb);
    },
    getTagByName: function(name, cb) {
