...
HabitRpg.prototype.createTask = function(task, cb) {
  var req = this.buildRequest('POST', '/user/tasks', task);
  req.end(cb);
};

HabitRpg.prototype.getTask = function(id, cb) {
  var req = this.buildRequest('GET', '/user/tasks/' + id);
  req.end(cb);
};

HabitRpg.prototype.getTasks = function(cb) {
  var req = this.buildRequest('GET', '/user/tasks/');
  req.end(cb);
};

HabitRpg.prototype.updateTask = function(id, task, cb) {
  var req = this.buildRequest('PUT', '/user/tasks/' + id, task);
  req.end(cb);
};

HabitRpg.prototype.deleteTask = function(id, cb) {
  var req = this.buildRequest('DEL', '/user/tasks/' + id);
  req.end(cb);
};
...
HabitRpg.prototype.updateTaskScore = function(id, direction, cb) {
  ...
  var req = this.buildRequest('POST', '/user/tasks/' + id +'/' + direction);
  req.end(cb);
};

HabitRpg.prototype.getUser = function(cb) {
  var req = this.buildRequest('GET', '/user');
  req.end(cb);
};
